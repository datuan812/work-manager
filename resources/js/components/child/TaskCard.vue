<script setup>
defineProps({
    dailyTask: { type: Object, required: true },
    busy: Boolean,
    readonly: Boolean,
        ...
        :class="
            dailyTask.status === 'completed'
                ...
                : dailyTask.status === 'incomplete'
                  ...
                  : 'border-white/80 bg-white/[0.82] backdrop-blur'
        "
    ...
    >
        <span class="min-w-0">
            ...
            <span class="mt-2 inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-bold uppercase text-slate-500">
                {{ statusLabel || (dailyTask.status === 'completed' ? 'Hoàn thành' : dailyTask.task.category?.name || 'Nhiệm vụ') }}
            </span>
        </span>
            ...
            <span
                ...
                :class="
                    dailyTask.status === 'completed'
                        ...
                        : dailyTask.status === 'incomplete'
                          ? 'border-amber-300 bg-amber-100 text-transparent'
                          : 'border-slate-300 bg-white/70 text-transparent group-hover:border-sky-300'
                "
            >✓</span>
        </span>
    </button>
