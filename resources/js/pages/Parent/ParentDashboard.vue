    ...
    <ParentLayout>
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h1 class="mt-1 text-3xl font-black">Tổng quan hôm nay</h1>
            </div>
        </div>
            ...
            <article v-for="child in parent.dashboard?.children" :key="child.id" class="admin-card p-5">
                <div class="flex items-center justify-between gap-3">
                    <div class="flex items-center gap-3">
                        ...
                        <h2 class="text-xl font-black">{{ child.name }}</h2>
                    </div>
                    <span class="rounded-full px-3 py-1 text-xs font-black" :class="child.is_active ? 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100' : 'bg-slate-100 text-slate-500'">
                        {{ child.is_active ? 'Active' : 'Inactive' }}
                    </span>
                </div>
                <p class="mt-5 text-sm font-black text-slate-600">{{ child.today_completed }} / {{ child.today_total }} nhiệm vụ hoàn thành</p>
                <ProgressBar class="mt-2" :value="child.completion_percent" />
                <div class="mt-4 flex justify-between text-sm font-black text-slate-600">
