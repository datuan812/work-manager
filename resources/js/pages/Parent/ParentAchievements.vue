...
<template>
    <ParentLayout>
        <div>
            ...
            <h1 class="mt-1 text-3xl font-black">Thành tựu</h1>
        </div>
        ...
        <div v-else class="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <article v-for="achievement in parent.achievements" :key="achievement.id" class="admin-card p-5">
                <div class="flex items-start justify-between gap-4">
                    ...
                    <span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-600">{{ achievement.users_count }} unlocked</span>
                </div>
                <h2 class="mt-4 font-black">{{ achievement.title }}</h2>
